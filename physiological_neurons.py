...
but just as the same neurons with additional input
...
def create_noise_vector(N, fraction):
  ...
  for i in range(N):
    p = np.random.rand()
    if (p < fraction):   #if rando value < fraction => neuron is noisy-active !
      nv[i] = 1
  return(nv)
...
def one_step_LIF_noise(state, input_matrix, voltage,syn_del, W_syn,  edges, t, parameters, noise_freq, noise_vector):
    ...
    for i in range(n_neurons):
        ...
        statei = state[i,t]  #retrieves the state of neuron i at time t (prev_time) from the state matrix.
        n_neighbors = int(edges[i,0])  # number of nb
        W_synapse = np.zeros(n_neighbors)  
        out = np.zeros(n_neighbors)
        constant = statei*np.exp(-statei / tau_s)  

        # loop thru the neighbors of neuron i
        for j in range(n_neighbors):
            out[j] = W_syn[i,j]*constant
            neighbor = int(edges[i, j+1]) #get the actual nb stored at edges[i][j+1]
            delay = int(syn_del[i,j])
            #output from i-> j is now passed to the neighbor at time t + delay + 1
            input_matrix[neighbor, t + delay + 1] = input_matrix[neighbor, t + delay + 1] + out[j]
    for i in range(n_neurons):
      if (noise_vector[i] == 1):
...
def full_process(edges, syn_del, input_matrix, W_matrix,  par,  noise_vector):
    ...
    stepsize = par['stepsize']   #0.05 ms
    n_timesteps = int(par['n_timesteps'])  #1000
    tau_s = int(par['tau_s'])

    #states = matrix 1000*num_timesteps => see how state change over time
    # initilze to 1000*taus
    states = np.zeros(n_neurons*n_timesteps).reshape(n_neurons, n_timesteps)
    for i in range(n_neurons):
        for j in range(n_timesteps):
            states[i,j] = 1000 * tau_s  #high memrane potential => hard to depolarize => won't fire!
    max_del = np.max(syn_del.reshape(n_neurons**2))  #find max synaptic delay
    V_threshold = par['V_threshold'] 
    #voltage = matrix of volts within neurons throughout time
    voltages = np.zeros(n_neurons*n_timesteps).reshape(n_neurons, n_timesteps)
    t = 0
    av_V = 0
    while ((t < n_timesteps - 1 - max_del) and (av_V < V_threshold)):
       #keep changing states, voltages, t until we reach threshold or run out of time
        ...
        sum_V = 0
        for i in range(n_neurons):
            sum_V = sum_V + voltages[i,t]  #voltage[i][t] = voltage of neuron i at time t
        av_V = sum_V / n_neurons    
        #at each time step, we get a new sum and thus a new average of voltage.
