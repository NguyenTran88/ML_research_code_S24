        ...
        for j in range(N):
            if (i == j):
                ...
                if (M[i,j] < pct_connected):
                    M[i,j] = 1 #i connects to j if m[i][j] < 0.065; m[i][j] is random btw 0->1
                else:
                    M[i,j] = 0
    return M
...
@jit(nopython=True)
def matrixOfEdges(M,N):  #M is an adj matrix, N is # of vertices in the graph
    N = int(N)
    E=np.zeros(N**2).reshape(N,N)  #prepare 2D array with zeros
    k=np.sum(M,0) #k is array with degree of each vertex
...
#E is a matrix of edges: each row of E represents a vertex, and the row holds the list vertices connected to the vertex
...
def prepare_syndel_2(average, std, edges, n_neurons ):
    ...
    for i in range(n_neurons):
      neigh = int(edges[i,0]) # number of neighboring vertices
      for j in range(neigh):
        synaptic_del[i,j] = np.random.uniform(average - std, average + std)
        #random uniform distribution of synaptic delays within 1 std dev from mean 
    return(synaptic_del)

def active_set(N, n_activated):  #N = 1000; n_active = 7
  set_activated = np.zeros(n_activated)
  for i in range(n_activated):
    a = np.random.randint(N)
    for j in range(i):
      while (a == set_activated[j]):
        a = np.random.randint(N)  #randomise until a != previous activated neuron => 
    set_activated[i] = a
  return(set_activated)    # set of 7 number in range 1000
...
def generate_network(params, network_type = 'ER'):
    if (network_type=='ER'):
        ...
        sigma = params['sigma']
        M = getRandomConnectivity(n_neurons,p)   #1000, 0.065
        ...
        syn_del = prepare_syndel_2(avdel, stddel, edges, n_neurons )
